...
function cleanHtml(html: string): string {
  let cleaned = html
    ...
    .replace(/<!--[\s\S]*?-->/g, "")
    // Strip small forms (< 5000 chars)
    .replace(/<form(?![\s\S]{5000})[\s\S]*?<\/form>/gi, (match) => match.length < 5000 ? "" : match)
    // Strip unnecessary HTML attributes (keep href, src, alt, data-lazy)
...
async function upsertVehicles(
  ...
  dealershipId: string,
  scrapeUrl: string
): Promise<{ newCount: number; updatedCount: number; markedSold: number; savedCount: number }> {
  let newCount = 0;
  let updatedCount = 0;
  let markedSold = 0;
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // ══════════════════════════════════════════════════════
    // MODE 3: Save pre-enriched vehicles (client did image fetching)
    // ══════════════════════════════════════════════════════
    if (vehiclesEnriched && vehiclesEnriched.length > 0) {
      console.log(`Saving ${vehiclesEnriched.length} pre-enriched vehicles with images`);

      const { newCount, updatedCount, markedSold, savedCount } = await upsertVehicles(
        supabase, vehiclesEnriched, dealershipId, scrapeUrl
      );

      const totalImages = vehiclesEnriched.reduce(
        (sum: number, v: any) => sum + (Array.isArray(v.images) ? v.images.length : 0), 0
      );

      await supabase.from("dealer_settings").update({
        scraper_status: "idle",
        scraper_last_run: new Date().toISOString(),
        scraper_vehicle_count: savedCount,
      }).eq("dealership_id", dealershipId);
      ...
      try {
        await supabase.from("ingestion_logs").insert({
          ...
          vehicles_scanned: vehiclesEnriched.length,
          new_vehicles: newCount,
          marked_sold: markedSold,
          images_fetched: totalImages,
          status: "success",
          message: `Scraped ${scrapeUrl} — ${pagesScrapedOverride || "?"} pages, ${newCount} new, ${updatedCount} updated, ${markedSold} marked sold, ${totalImages} photos`,
        });
      } catch (_e) { /* ingestion log is non-critical */ }
      ...
      return new Response(JSON.stringify({
        ...
        marked_sold: markedSold,
        pages_scraped: pagesScrapedOverride,
    ...
    // ══════════════════════════════════════════════════════
    if (skipDetailFetch) {
      // Don't save yet — return vehicles_raw so client can enrich with images
    ...
    // ══════════════════════════════════════════════════════
    // MODE 1: Full server-side flow (extract + save immediately)
    // ══════════════════════════════════════════════════════
    const { newCount, updatedCount, markedSold, savedCount } = await upsertVehicles(
      supabase, vehicles, dealershipId, scrapeUrl
    );
    ...
    const totalImages = vehicles.reduce(
      (sum: number, v: any) => sum + (Array.isArray(v.images) ? v.images.length : 0), 0
    );
    ...
    console.log(`Scrape complete: ${savedCount} found (${pagesScraped} pages), ${newCount} new, ${updatedCount} updated, ${markedSold} sold`);
